...
class RUNNER():
    ...
    def update_path_history(self):
        ...
        if location in self.history:
           ...
           print ('you have been at %s before! %d steps' % (str(location), self.answer))
        else:
    ...
    def find_shortest_path(self):
      ...
      directions = ['','']
      directions[0] = 'Walk %s%d' % ("North" if self.distance[0] >= 0 else "South", self.distance[0])
      directions[1] = 'Walk %s%d' % ("East" if self.distance[1] >= 0 else "West", self.distance[1])
